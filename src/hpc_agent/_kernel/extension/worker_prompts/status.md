...
- **Wait until terminal (or budget)** → invoke the [status-pipeline](../../docs/primitives/status-pipeline.md) composite. It runs [monitor-flow](../../docs/primitives/monitor-flow.md) to terminal/budget and folds the `lifecycle_state` dispatch into one typed `stage_reached` ∈ `{complete, timeout, failed, abandoned}` plus a `needs_decision` flag (escalation-as-data) — so you read one outcome instead of hand-branching the lifecycle. Use when the caller wants to wait synchronously for a run to finish (the canonical campaign-loop case).
