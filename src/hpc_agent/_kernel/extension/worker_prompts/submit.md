...
**Do NOT add `"runtime": "uv"` by default** — most clusters don't have `uv` on PATH, and the cluster preamble's "`HPC_RUNTIME=uv but 'uv' not on PATH`" guard fails every task in the array. Only set `runtime: "uv"` when the repo genuinely depends on `uv sync` (a `pyproject.toml` with a uv lock file AND the operator has confirmed the cluster conda env carries `uv`). When in doubt, omit it; the submit-flow's preflight (`command -v uv` against the cluster env, #275) refuses the spec at preflight if you set it on a uv-less cluster, but the friendlier path is "don't set it in the first place."
