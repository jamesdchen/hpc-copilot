...
This step exists for the **wrapper fallback**: when direct decoration wasn't possible (non-Python entry point, decorator conflict, vendor code), `/wrap-entry-point-hpc` materializes a wrapper at `.hpc/wrappers/<run_name>.py` and writes `interview.json` with a `shell_command` `_materialized.entry_point`. **Read `interview.json` with the Read tool** (do not shell `python -c`).
...
If `main.py` is present and `@register_run` is found nowhere: record in `anomalies` (prefix `mature_repo_needs_interview: main.py present, no @register_run; ask the user to add @register_run to their entry-point function (two-line edit: import + decorator), or run /wrap-entry-point-hpc for guided setup, then re-invoke`) and stop.
