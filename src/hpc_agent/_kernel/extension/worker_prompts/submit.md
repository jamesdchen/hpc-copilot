...
The canary is a GATE (#160): the 1-task canary is submitted, verified to land and produce output, and the main array launches ONLY on success — all inside the primitive. The Phase-2 flips (`canary`/`canary_only` off, `skip_rsync_deploy` on — #185/#279) are applied inside it.
