Agent-facing composition over the **[submit-flow](../../docs/primitives/submit-flow.md) workflow atom**. For just the journal-write half (when the agent already qsubbed), use [submit-spec](../../docs/primitives/submit-spec.md) directly. Both are idempotent on `run_id`: a replay returns `data.deduped: true`.

"Invoke <primitive>" means call its `backed_by.cli` or `backed_by.python` entry point; see `docs/primitives/<name>.md`. For envelope/exit-code shapes see `docs/reference/cli-spec.md`.
...
**Step 0 — cluster-SSH preflight, before any heavy local prep (#265).** Fail fast on a sandboxed-network block. As your first cluster-touching action — right after `load-context` resolves the cluster, before any interview/classify/build — run `hpc-agent check-preflight --cluster <cluster>`. If blocked in a way consistent with a sandboxed network (the probe cannot leave the sandbox), STOP and return `spec_invalid: sandbox_blocks_cluster_ssh` (`ok: false`) with remediation: *"this workflow needs un-sandboxed cluster SSH; re-run as the default `--bare` spawn (set `ANTHROPIC_API_KEY` so it can authenticate) instead of inline, or disable the session sandbox."* Do **NOT** report `ok: true` with a buried "SSH was blocked" note.
...
`src/` is `.gitignore`d. Build it from the notebooks before discovery, the elision gate, or the deploy bundle:

```bash
hpc-agent export-package --experiment-dir .
```

Exports `notebooks/{pipeline,executors,scripts}/*.ipynb` → `src/<module>.py`, content-hash-cached against `.hpc/.build-cache.json` (no-op when nothing changed). The built `src/` rides `submit-flow`'s rsync; **the cluster node never builds**. On `spec_invalid` (output-path collision, bad module name), surface and stop — the notebooks need a rename.

## Step 0b: Honor a `shell_command` wrapper (fallback path only)

**The default mature-repo path is `@register_run` on the user's function** — Step 1's `discover_runs` finds it in `notebooks/`, `train.py`, or `main.py`. No special branch needed.
...
Its `data` carries the repo scan (`kind`, `candidates`, `decoration_found`) **and** the optional `materialized` block — the `_materialized.entry_point` from `interview.json` when the wrap workflow already chose an entry point. If `data.materialized` is present, branch on `data.materialized.kind`:

| `kind` | Procedure behavior |
|---|---|
| `shell_command` | **The fallback path.** A wrapper has been materialized at `data.materialized.wrapper_path` (`.hpc/wrappers/<run_name>.py`) — it satisfies the `@register_run` contract for an entry point the framework can't decorate directly. **Skip Step 1's discover scan**; treat `data.materialized.run_name` as the picked run. **Use `data.materialized.executor_cmd` as the `EXECUTOR` in Step 6's `job_env`** instead of synthesizing one from `discover_executors`. If `data.materialized.data_axis` is present, **skip the Step 3b classification interview** — the user pre-declared the axis; just feed `data_axis` into the `axes.yaml` write that classify-axis would have done. `tasks.py` is already on disk (the interview materialized it from `task_generator`) — Step 6's reuse branch (omit `build_tasks`) picks it up. |
| `register_run` | A pointer to a `@register_run`-decorated function the user has on disk — the canonical Python path. No wrapper to honor; fall through to Step 1's discovery, optionally scoped to `data.materialized.run_name` rather than enumerating. |
| `python_module` | An importable `data.materialized.module`:`data.materialized.function` the framework targets by dotted path — there is **no `@register_run` to discover**, so **skip Step 1's discover scan** and treat `function` as the picked run. **Do NOT synthesize an `EXECUTOR`**: the per-task command is `data.materialized.executor_cmd` (`python3 -m hpc_agent.executor_cli run-module <module>:<function>`), and `resolve-submit-inputs` writes it into the sidecar **deterministically from the materialized block** — the worker never divines `python3 -m <module>`. `tasks.py` is already on disk (the interview materialized it from `task_generator`) — Step 6's reuse branch (omit `build_tasks`) picks it up. |

`register_run` is the one pointer the worker resolves through normal Step 1 discovery (its function carries `@register_run`); `shell_command` and `python_module` **short-circuit discovery** — the materialized wrapper / dotted module is named directly. For both of the latter the per-task executor comes from `data.materialized.executor_cmd`, which `resolve-submit-inputs` writes into the sidecar **in code** — the worker does not hand-build it (a python_module entry that shipped a bare `module:function` was the ridge_imp exit-127 incident).

If `data.materialized` is absent (no `interview.json`, or no `_materialized.entry_point`), **read the same envelope's repo scan to decide whether this is a mature-repo case** that needs an interview the headless worker can't conduct:

- `HAS_MAIN`: a `candidates` entry whose `path` is `main.py` or `src/main.py`.
- `HAS_REGISTER_RUN`: a non-empty `decoration_found` (or any `candidates` entry already decorated).

If `HAS_MAIN` holds and `decoration_found` is empty: record in `anomalies` (prefix `mature_repo_needs_interview: main.py present, no @register_run; ask the user to add @register_run to their entry-point function (two-line edit: import + decorator), or run the entry-point wrap workflow for guided setup, then re-invoke`) and stop.

Otherwise (no mature-repo signals): the rest of the procedure runs unchanged.

## Step 1: Discover runs

**Function-first.** The researcher's contract is a `@register_run def run(...)`. It may live in a notebook, a script, or a package module. Invoke [discover-runs](../../docs/primitives/discover-runs.md) — it AST-walks all three (skipping `.hpc/`):
...
- **Unscoped submit** (the default) — list every `@register_run` and let the user pick one.
- **Submit scoped to one file** (a file path arrived in the invocation `fields`) — scope discovery to that one file (a notebook or a script).
...
The task list lives in user-written `.hpc/tasks.py` (`total()` + `resolve(task_id)`). Step 6 scaffolds it once; from then on it is committed and reused. Two shapes:
...
> **`DataAxis` ≠ scheduling axes.** `axes.yaml` holds two unrelated things: `executors.<run>.data_axis` (this step — *how to split the series*) and `homogeneous_axes` / `axes` (Step 4b / `hpc-axes-init` — *which sweep dimension goes on the task array*). Orthogonal.

### 3c: Serial-elision gate (mandatory for a splittable axis — `independent` / `associative` / `bounded_halo`)

Before scaffolding a planner-driven `tasks.py`, prove the classification: `hpc_agent.experiment_kit.check_elision` (or `assert_elision_equivalent`) runs the experiment once whole and once split N ways and asserts the results agree. If it fails, the axis is misclassified — widen the halo or fall back to `Sequential()`. Do not skip; recommend wiring `assert_elision_equivalent` into the repo's CI.

If the projected task count exceeds `constraints.max_tasks` or ~1000, record a `magnitude_warning` in `anomalies`.
...
```json
{
  ...
  "sidecar": {
    ...
    "cluster": "<cluster>", "profile": "<run_name>", "campaign_id": "<slug>"
  },
  "build_tasks": { "...ONLY when .hpc/tasks.py is absent: axes + flags_by_executor + (data_axis from Step 3's classification)..." }
}
```

**Do NOT add `"runtime": "uv"` by default** — most clusters don't have `uv` on PATH, and the preamble's `HPC_RUNTIME=uv but 'uv' not on PATH` guard fails every task. Only set it when the repo depends on `uv sync` (a `pyproject.toml` with a uv lock file AND the operator confirmed the cluster conda env carries `uv`). When in doubt, omit.
...
- `needs_scaffold_interview` (`needs_decision=true`) → `.hpc/tasks.py` is absent and no `build_tasks` scaffold spec was supplied — scaffolding needs an executor-discovery + axes interview the headless worker can't run. Record it in `anomalies` (prefix `mature_repo_needs_interview:`) and stop; the user adds `@register_run` / runs the entry-point wrap workflow, or resolves the axes upstream, then re-invokes.
...
The post-resolution submit spine — canary-gated submit → post-qsub health check → follow-up-spec pre-staging — runs as ONE call to [submit-pipeline](../../docs/primitives/submit-pipeline.md).

The canary is a GATE (#160): the 1-task canary is submitted, verified to land and produce output, and the main array launches ONLY on success — all inside the primitive. The Phase-2 flips (`canary`/`canary_only` off — #279) are applied inside it; the redundant rsync+deploy is skipped in-process too (Phase 1 just deployed the same tree), which is operator/internal-only and NOT a spec field you set (#283/#185).

The spec embeds the canary-gated submit under `submit` plus an optional `profile` forwarded to follow-up staging. It matches `schemas/submit_pipeline.input.json`:
...
There is **no `skip_preflight`** field (#275 Fix 2): preflight runs inside `submit-flow` and is skippable only by the operator env var `HPC_AGENT_SKIP_PREFLIGHT`. For GPU jobs: `submit.submit.script: ".hpc/templates/gpu_array.sh"` (SGE) or `gpu_array.slurm` (SLURM).

```bash
hpc-agent submit-pipeline --spec spec.json --experiment-dir .
```

Run this call **foreground with an explicit Bash `timeout: 600000`** — the pipeline waits out the canary in-process (minutes by design), and an un-timeouted call is auto-backgrounded by the harness at 2 minutes. If it is backgrounded or times out anyway: poll the task's output until the single JSON envelope appears, and branch on that envelope. **Never end your run while the pipeline is still executing** — your exit kills it (empirical 2026-06-11: killed between the main-array qsub and the journal write; the job id was lost and the run became untrackable). **Never report success from side-state**: a canary journal entry or `load-context` showing the canary in flight proves only that the canary fired — the pipeline envelope is the only result.
...
- `complete` (`needs_decision=false`) → the canary passed, the main array is live and healthy, and the follow-up specs are pre-staged (`data.monitor_spec_path` / `data.aggregate_spec_path`, #278 — so the status and aggregate workflows skip their interview round-trip, each `cmd_sha`-gated against the journal). Capture `data.run_id` / `data.job_ids` and report.
- Error envelopes: branch by `error_code` per the primitive's contract.
...
Report after a `complete` stage: job ID, executor(s), grid dimensions, total tasks, cluster, verified scheduler state. The caller switches to the status workflow to track progress.
