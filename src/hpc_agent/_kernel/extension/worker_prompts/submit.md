...
- **`decisions`** is the **strict enumerated record** of which judgement points this workflow reached. For the **submit** workflow there are exactly seven allowed `point` IDs — any other value is rejected by `parse_worker_report`:
  ...
  Each entry is `{point, outcome, why}` — `outcome` is a short tag describing what happened at that point (e.g. `unclassified`, `setup_required`, `fail`, `dispatcher_failed`), `why` is a free-form one-liner.
...
- `found=True, is_orphan=False` → real prior. Record a `prior_run` decision with outcome `found` (put the `run_id` in `why`) and surface to the caller — only the user can choose resume-vs-fresh.
...
Otherwise invoke [check-preflight](../../docs/primitives/check-preflight.md) with `--cluster <name>`. On `data.all_ok == true`: write/update marker, continue. On any check failure: do NOT write marker, record a `preflight` decision with outcome `setup_required`, put the failing checks verbatim in `anomalies`, and stop — the user fixes their environment with `hpc-agent setup --cluster <name>` and the caller re-invokes.
