...
def render_spawn_parts(
    *, workflow: str, experiment_dir: str, fields: dict[str, Any]
    ...
    cacheable_prefix = (
        ...
        "- When it cross-references another skill or a primitive (`hand "
        "off to the hpc-status skill`, a `docs/...` link), you have no "
        "Skill tool but you CAN read the content: `hpc-agent describe "
        "<name>` prints a skill's procedure or a primitive's contract "
        "from the installed package, and `hpc-agent <primitive> --help` "
        "gives a primitive's exact CLI contract. Fetch only what the "
        "branch you are on actually needs, and follow it inline. The one "
        "thing you cannot do headless: a referenced skill that needs "
        "interactive user confirmation (e.g. `hpc-classify-axis`) — "
        "record it in `decisions` / `anomalies` and stop at that boundary "
