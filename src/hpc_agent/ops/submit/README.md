...
`ops/submit/` owns the path from a typed submit spec to an SSH-pushed cluster job. It packs the atom (`plan_summary.py` — the deterministic pre-submit summary string), the throughput planner primitive (`plan_throughput.py` — wraps the domain-neutral packing algorithm in `infra/throughput.py` as the `plan-throughput` primitive), the partition advisor (`recommend_partition.py` — the `recommend-partition` primitive), and the runner (`runner.py` — `submit_and_record` writes the journal sidecar around an SSH-issued qsub). The `submit-flow` / `submit-flow-batch` workflow composite lives at `ops/submit_flow.py` (role-root sibling per P5a) — it ties pre-flight, rsync, deploy, optional canary, qsub, and journal write into one envelope.
...
- `recommend_partition.py` — agent-facing pure helper (primitive: `recommend-partition`).
