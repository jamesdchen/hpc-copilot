...
A :data:`~hpc_agent.template.axis.DataAxis` is a live object — and for
:class:`~hpc_agent.template.axis.BoundedHalo` it carries a *callable*
...
It uses only the stdlib (:mod:`ast`) and :mod:`hpc_agent.template.axis`
(itself stdlib-only).
"""
...
from hpc_agent.template.axis import (
    MOMENTS,
    SUM,
    Associative,
    BoundedHalo,
...
def config_from_data_axis(axis: DataAxis) -> dict[str, Any]:
    ...
    :class:`~hpc_agent.template.axis.BoundedHalo` only round-trips if its
