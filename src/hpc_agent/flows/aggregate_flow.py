...
def _incremental_include_patterns(
    ...
    * No combiner files are present locally yet (first call) AND every
    ...
    Otherwise returns a list of ``wave_<N>.json`` /
    ``wave_<N>.runtime.json`` include patterns sized to the diff. An
    empty diff (everything already pulled) returns a single non-matching
    sentinel pattern so rsync's filter excludes every file — the rsync
    still runs (cheap directory stat confirms reachability) but
    transfers nothing.
    """
...
def _incremental_include_patterns(
    ...
    * No combiner files are present locally yet (first call) AND every
    ...
    pulled) returns a single non-matching sentinel pattern so rsync's
    filter excludes every file — the rsync still runs (cheap directory
    stat confirms reachability) but transfers nothing.
    """
    if not combined_waves:
        return None
    have_locally: set[int] = set()
...
def aggregate_flow(
    ...
    *,
    ...
    include_patterns = _incremental_include_patterns(
        combiner_local, list(record.combined_waves)
    )
    pull = rsync_pull(
        ssh_target=record.ssh_target,
        remote_path=record.remote_path,
