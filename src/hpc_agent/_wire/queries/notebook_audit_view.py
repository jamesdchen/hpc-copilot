...
class NotebookAuditViewSpec(BaseModel):
    ...
    full: bool = Field(
        ...
        description=(
            ...
            "the bodies live in the content-addressed render files (`render_path`), "
            "so the loop's routine passes need the digest + pointers only. "
            "Pass `full: true` for the INLINE read-and-sign `markdown` — the "
            "review surface the agent relays VERBATIM in chat: the code diff with "
            "its highlighting intact, commented-out exposition runs collapsed to "
            "disclosed elision lines (the full exposition stays in the on-disk "
            "render for out-of-chat auditing). The per-section view_shas and "
            "`render_path`s are identical "
...
class NotebookAuditViewResult(BaseModel):
    ...
    markdown: str = Field(
        description=(
            ...
            "files). Pass spec `full: true` "
            "for the inline read-and-sign render (code diff intact; commented-out "
            "exposition elided with disclosure — the on-disk render keeps the full "
            "text). Same view + same `full` → byte-identical "
            "markdown."
        ),
    )
