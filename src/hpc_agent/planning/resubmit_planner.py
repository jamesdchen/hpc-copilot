...
* :func:`~hpc_agent.atoms.walltime_arbitrage.arbitrage_walltime`
  — trim the walltime ask to fit backfill shadows when no prior exists.
* ``cold_start_mem_buffer`` from :mod:`hpc_agent.infra.clusters` — grow
  the memory ask by N% when no prior exists, clamped by the cluster's
  ``max_node_mem_mb`` ceiling.
* :func:`~hpc_agent.planning.daisy_chain.should_daisy_chain` — flag
...
:mod:`hpc_agent.forecast.calibration`).
...
from hpc_agent.atoms.walltime_arbitrage import arbitrage_walltime
...
from hpc_agent.planning.daisy_chain import should_daisy_chain
