...
def _render_literal_tasks(tasks: list[dict[str, Any]]) -> str:
    ...
    then carries no ``hpc_agent.incorporation.template`` import, so it imports cleanly
...
def _build_data_axis(data_axis: dict[str, Any]) -> Any:
    ...
    Used at scaffold time (on the laptop, where ``hpc_agent.incorporation.template`` is
    ...
    imports ``hpc_agent.incorporation.template``.
    """
...
The series axis was partitioned by ``hpc_agent.incorporation.template.plan_tasks`` at
...
``hpc_agent.incorporation.template.load_series`` consumes in the executor.
...
gate — see ``hpc_agent.incorporation.template.check_elision``. A misclassified axis
...
def build_tasks_py(
    ...
    *,
    ...
    data_axis:
        ...
        :func:`hpc_agent.incorporation.template.plan_tasks` per the classified
    ...
    if data_axis is not None:
        ...
        # generated file, so it carries no hpc_agent.incorporation.template import.
        from hpc_agent.incorporation.template import plan_tasks
