...
def _render_literal_tasks(tasks: list[dict[str, Any]]) -> str:
    ...
    then carries no ``hpc_agent.experiment_kit`` import, so it imports cleanly
...
The series axis was partitioned by ``hpc_agent.experiment_kit.plan_tasks`` at
...
``hpc_agent.experiment_kit.load_series`` consumes in the executor.
...
gate — see ``hpc_agent.experiment_kit.check_elision``. A misclassified axis
...
def build_tasks_py(
    ...
    *,
    ...
    data_axis:
        ...
        :func:`hpc_agent.experiment_kit.plan_tasks` per the classified
    ...
    if data_axis is not None:
        ...
        # generated file, so it carries no hpc_agent.experiment_kit import.
        from hpc_agent.experiment_kit import plan_tasks
