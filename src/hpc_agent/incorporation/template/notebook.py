...
``hpc_agent.incorporation.template`` (i.e. it is a ``@register_run`` experiment), the
export runs in *inline mode*: the ``hpc_agent.incorporation.template`` import is
dropped and the source of :mod:`hpc_agent.incorporation.template._runtime` — the
...
_RUNTIME_MODULE = "hpc_agent.incorporation.template"
...
_INLINE_HEADER = (
    "# ─── inlined hpc_agent.incorporation.template runtime "
...
def export_notebook(ipynb: str | Path, out_py: str | Path) -> Path:
    ...
    if inline:
        # Drop the hpc_agent.incorporation.template import (the inlined shim provides
...
def notebook_imports_runtime(ipynb: str | Path) -> bool:
    """True if any code cell of *ipynb* imports from ``hpc_agent.incorporation.template``.
...
def _imports_runtime(node: ast.stmt) -> bool:
    """True if *node* imports from ``hpc_agent.incorporation.template`` (any submodule)."""
