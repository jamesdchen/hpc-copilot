...
class JournalStatus(StrEnum):
    """Status field on :class:`hpc_agent._internal.session.RunRecord`.
...
# ``hpc_agent._internal.session`` historically exposed this same set as a
...
class TaskStatus(StrEnum):
    """Per-task status reported by :mod:`hpc_agent.mapreduce.reduce.status`.
