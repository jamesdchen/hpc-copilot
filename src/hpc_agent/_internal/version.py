...
Three JSON shapes in the codebase carry a ``schema_version`` field and
...
* ``hpc_agent/mapreduce/reduce/status.py`` — status rollup (``schema_version = 2`` literal)
...
def compatibility_check(domain: str, found: int) -> None:
    ...
    *domain* must be a key in :data:`_MANIFEST`; otherwise a
    ...
    """
    # Local import: ``hpc_agent.errors`` imports nothing else from
    # ``hpc_agent`` so this is safe, but the import is inside the
