...
back to ``~/.claude-hpc/config.json:experiment_roots``. Both empty
...
_USER_CONFIG = Path("~/.claude-hpc/config.json").expanduser()
...
def resolve_roots(explicit: str | None) -> list[Path]:
    ...
    * Else read ``~/.claude-hpc/config.json:experiment_roots``; return
