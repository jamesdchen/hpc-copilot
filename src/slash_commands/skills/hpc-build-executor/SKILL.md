...
2. Invoke as your FINAL tool call:
   ...
   The verb validates against `hpc_agent/schemas/skill_returns/hpc-build-executor.json` and atomically renames `.staged.json` → `.json`. Then **hand control back to the parent without ending your turn** — emit no summary or closing message. The parent's next action is `hpc-agent fetch-skill-return --skill hpc-build-executor`.
