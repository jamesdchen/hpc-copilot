...
- **terminal-complete** (`complete` / `timeout` / …) — the cluster confirms the run finished with results on disk; the journal is now marked terminal. Treat this run as the terminal `run_id` and continue to Step 2/3. (`failed` is terminal too but is NOT aggregatable — see its dedicated branch below.)
- **`failed`** (#351) — recorded `job_ids` exist, none are alive, AND the reporter showed positive failure evidence (`failed >= 1`: a task reached the cluster, ran, and exited non-zero). This is NOT an abandon — the readable error is on disk. `data.last_status.failure_features` carries the structured evidence (`cluster_log_tail`, `log_path`, `classified_error` = `{error_class, suggested_fix, matched_pattern}`). Return `spec_invalid` naming the `run_id` with `verdict: "canary/run failed"` and surface `failure_features` verbatim (especially `classified_error.suggested_fix` and the raw `cluster_log_tail` — do NOT paraphrase the log). Remediation: fix the surfaced error, then re-submit. Do NOT report `run_abandoned` (the run did not vanish; it failed) and do NOT report "nothing to aggregate".
- **`abandoned`** — recorded `job_ids` exist but none are alive on the scheduler AND there is no failure evidence on disk either (scratch wiped, job manually cancelled, scheduler retention purged the record — `failed == 0`). Return `spec_invalid: run_abandoned` naming the `run_id`, with remediation: re-submit the run, or — if the cluster `_combiner/` partials still exist — aggregate them explicitly via `mode: "combiner-only"`. Do NOT silently report "nothing to aggregate".
