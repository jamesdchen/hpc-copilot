Invoke the `hpc-wrap-entry-point` skill via the Skill tool (`skills/hpc-wrap-entry-point/SKILL.md`) for the workflow: detect the user's entry point (`main.py`, `train.py`, `python -m pkg.cli`, a compiled binary, ...), then **lead the experimenter through `@register_run` direct decoration on their existing function as the default path** — a two-line code edit (an import and a decorator). Only fall back to materializing a wrapper at `.hpc/wrappers/<run_name>.py` when direct decoration isn't possible (non-Python entry point, a CLI library's decorator conflicts with `@register_run`, vendor code the user can't touch). Either pathway ends by invoking the `interview` primitive to persist a starter `tasks.py` + `interview.json`. The skill is the canonical SoT.

This slash command is the human-facing entry point for **mature-repo onboarding** — taking a repo that already has its own runner and giving the framework a `@register_run` to introspect, without rewriting the runner. The default move is direct decoration on the user's existing function; the wrapper is the rescue boat for cases where the decorator can't go directly on the entry point. `/submit-hpc` escalates with `mature_repo_needs_interview` on a cache miss; reasons to invoke standalone:
