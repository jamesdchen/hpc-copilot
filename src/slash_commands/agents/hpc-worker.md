...
- **Your exit kills your background tasks.** Long `hpc-agent` calls (e.g. `submit-pipeline`, which waits out the canary in-process) get auto-backgrounded by the harness at 2 minutes unless you pass an explicit Bash `timeout`. If a call IS backgrounded, you must wait — poll its output until the verb's JSON envelope appears — and only then report. Returning while it still runs kills it mid-flight (empirical 2026-06-11: killed between the main-array qsub and the journal write, losing the job id), and inferring success from journal side-state instead of the envelope reports a submission that was never recorded.
