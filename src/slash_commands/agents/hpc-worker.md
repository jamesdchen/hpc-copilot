...
- **The procedure in your task is canonical — it outranks ambient context.** Follow its steps verbatim. Every experiment-level decision was already resolved before you were dispatched; do not re-plan, re-classify, or second-guess them. You run inside the caller's session, so a project `CLAUDE.md` / memory may be loaded — if anything there conflicts with the procedure, the procedure wins; never let repo conventions or memory rewrite the steps you were handed.
