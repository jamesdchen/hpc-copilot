...
- **`audited_source`** (opaque, notebook-audit T14): the sidecar echo of `interview.json`'s `audited_source` opt-in block — `{source, template, audit_id}` (the render metadatum `rendered_notebook` is dropped). Core never interprets it; it is recorded verbatim so `export-dossier` can seal the complete audit trail (source `.py` + template `.py` + the notebook attestation journal). Normally **stamped in code** by `resolve-submit-inputs` from `interview.json`, not hand-authored. Absent (`null`) → the key is omitted, so a non-audited run's sidecar is byte-identical to one written before the field existed.
