...
Replays with the same `run_id` are no-ops: the call returns `deduped=true` and does not re-issue qsub. The wrapper `hpc_agent.ops.submit.runner.submit_and_record` is keyed on `run_id` (which is itself derived from `cmd_sha`, so a re-run of an unchanged `.hpc/tasks.py` produces the same `run_id`). Callers who see `deduped=true` should switch to `poll-run-status` rather than re-running the upstream qsub.
