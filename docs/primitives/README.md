...
Every primitive file ships YAML frontmatter with **behavioral metadata only**. Field-level contracts (input/output shapes) live in JSON Schemas under `claude_hpc/schemas/`; the primitive's `backed_by` field points at the schema-validated CLI/Python entry point.
...
| [campaign-advance](campaign-advance.md) | yes | _none_ | `` |
| [campaign-budget](campaign-budget.md) | yes | _none_ | `` |
| [campaign-converged](campaign-converged.md) | yes | _none_ | `` |
...
| [campaign-replay](campaign-replay.md) | yes | _none_ | `` |
...
| [prune-orphan-sidecars](prune-orphan-sidecars.md) | yes | removes-files: `<experiment>/.hpc/runs/*.json` | `` |
...
| [submit-flow-batch](submit-flow-batch.md) | yes | rsync: `<ssh_target>:<remote_path>`; scheduler-submit: `<cluster>`; writes-journal: `~/.claude/hpc/<repo_hash>/runs/<run_id>.json` | `` |
