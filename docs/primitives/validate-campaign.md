...
backed_by:
  ...
  python: hpc_agent.flows.validate_campaign.validate_campaign
---
# validate-campaign
...
**Schemas:** [`validate_campaign.input.json`](../../src/hpc_agent/schemas/validate_campaign.input.json), [`validate_campaign.output.json`](../../src/hpc_agent/schemas/validate_campaign.output.json). The `ValidatorFinding` shape is shared across every validator and lives at the top of the workflow's input schema's `$defs`.
