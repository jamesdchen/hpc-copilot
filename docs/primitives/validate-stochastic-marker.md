...
- **Not a wire contract for the slash command's UX** — the bug class it catches is invisible until the framework's dedup engages. Prose in the campaign worker prompt (`src/hpc_agent/_kernel/extension/worker_prompts/campaign.md`) describes the marker requirement; this atom mechanically verifies compliance.
