...
| 4 qsub dispatch→job-id window | `_remote_base._execute_command` | a way to inject a sever AFTER the scheduler accepts but BEFORE stdout's job-id reaches the client, plus a drivable job-name reconcile to prove no re-run + out-of-band id discovery. **The apex drill** — owner U3 (submit-once contract). Highest value; blocked on a submit-leg injection seam. |
