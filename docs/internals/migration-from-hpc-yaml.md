...
3. **For multi-stage `stages:` DAGs, port to `.hpc/stages.py`.** The
   helper `hpc_agent.state.stages.from_yaml_dict` is *not* shipped —
   ...
   `hpc_agent.state.stages.load_stages(experiment_dir)` — schema
...
2. **Closed-loop campaigns.** A new `campaign_id` field on the v2
   ...
   make the closed-loop pattern first-class. See `docs/internals/campaign.md` for
   the full feature.
