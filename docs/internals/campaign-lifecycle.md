...
The current answer: the loop is driven by `hpc-campaign-driver`, a
non-primitive console script that advances **exactly one step per
invocation** off a `delegate` block emitted by the `load-context`
primitive. Driver crashes, session restarts, machine reboots — none of
them matter, because every byte of state needed to resume lives on
disk in `.hpc/`, and the driver reconstructs it from scratch each
tick. Wrap it in cron, in `/loop`, in a CI job, in nothing at all — the
