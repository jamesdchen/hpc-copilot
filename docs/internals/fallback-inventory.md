...
|---|---|---|---|---|
| `infra/transport.rsync_push` (+ `disclose_push_mode` / `_disclose_no_rsync` / `_disclose_delta_mode` / `_disclose_payload`) | `_have_rsync()` false (no rsync on PATH, typically native Windows) | `tar c \| ssh tar x`; either a content-hash **delta** (remote hashes its own tree) or a full copy | Full re-ship of an identical tree (the 8.4 GB incident) | **DISCLOSES** — every push emits ONE `push MODE=<rsync\|delta-tar\|full-tar>` line naming the mode, the reason, and bytes-shipped vs bytes-unchanged. Note (2026-07-30): the old warn asserted a flat "NO-DELTA" cost and advised installing WSL/MSYS rsync; **both had rotted** — the content-hash delta IS the live rsync-less path (full copy is the narrow fallback: first ship / `delete=False` / `HPC_NO_DEPLOY_DELTA=1` / unusable manifest), and that rsync advice is the msys-2.0.dll runtime clash recorded at `infra/ssh_options.rsync_binary`. A transport-severed manifest probe is now REFUSED (stamped a transport flap for the staging retry) rather than silently degraded into a whole-tree re-ship. The reference implementation. |
...
| `ops/monitor_flow._announce_status` (crash-only Phase-2 announce-first census) | no announce dir yet — a pre-announce run, or a run no task has announced a terminal state for (the dispatcher creates the dir lazily on the FIRST marker); OR any ssh error on the census | the per-task status-reporter WALK (`record_status`) instead of the ONE-readdir marker census | the walk is the long-silent leg a NAT idle-drop / reaper severs at terminal (run-12 findings 20/24: a 20-25 min reporter walk severed mid-flight left a finished run unverifiable) — the census would have answered in one `ls` | **DISCLOSES** — a once-per-call stderr INFO names the reason at degrade time, AND every reporter-walk tick's `last_status` carries the in-band marker `status_source="status_reporter_walk"` (vs `"task_announcements"` on the census leg) recorded in the `.monitor.jsonl` tick log. The need decays as pre-announce runs age out. |
...
| `HPC_SSH_ENGINE` (unset/`asyncssh` → engine; `native` → one-shot) | default-on persistent connection; `EngineUnavailable` falls back to one-shot | JUSTIFIED-SILENT (behaviorally identical fallback). |
