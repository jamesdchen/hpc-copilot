...
|---|---|
| `uv run` for all Python | The integration runs `uv run hpc-mapreduce …` inside MARs's venv. Cluster-side dispatch honors the invariant when callers set `runtime: "uv"` on the submit spec — the agent then writes the per-run sidecar's `executor` field as `uv run python ...` (in `/submit` Step 6), and the four shipped templates run a `uv sync` preamble gated on `HPC_RUNTIME=uv`. See `docs/cli-spec.md` § submit. |
