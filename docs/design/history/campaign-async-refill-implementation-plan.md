...
per-repo `.submit_lock` (`ops/submit_flow.py::submit_flow_batch`) that serializes concurrent deploys —
...
- **RFC §5 (trial-tokens wiring)** — **already done.** `ops/resolve_submit_inputs.py::resolve_submit_inputs`
  injects both `trial_tokens` and `trial_params` into `sidecar_spec`. Write + read plumbing
  (`compute_run_id` → `write_run_sidecar` → `reduce/history.py::prior_records`) is fully wired;
...
- **File:** `src/hpc_agent/infra/io.py::advisory_flock` — its win32 branch (currently a
  permissions-only no-op).
...
### 1.1 Manifest fields
- **File:** `src/hpc_agent/_wire/fixtures/campaign_manifest.py::CampaignManifest` — add optional
  **top-level** fields: `async_refill: bool = False`,
...
- **File:** `src/hpc_agent/meta/campaign/atoms/advance.py`.
  - Add `_manifest_async_refill` / `_manifest_max_in_flight` helpers — mirror the *structure*
    of `_manifest_circuit_breaker_failures` (try/except + `isinstance` guard) but read
    **top-level** manifest keys (`manifest.get("async_refill")`), not `stop_criteria`.
  - Add `--async-refill` / `--max-in-flight` to the `CliShape` args tuple, the `campaign_advance`
    signature, and default-from-manifest (mirror the `_manifest_circuit_breaker_failures` wiring).
  - Add a `_refill` rule to `campaign_advance`'s `rules=[...]` ladder, ordered **after**
    `_over_budget` and all `stop_*` rules, **replacing `_wait_in_flight` only when `async_refill`
    is set**. Compute `refill_count = max(0, min(K, remaining_max_jobs) - in_flight)` where
    ...
    `decide_concurrency::decide_concurrency`'s `safe_bound` (currently unused by the driver). Carry
    `refill_count` in `CandidateAction.params`.
  - Surface `decision="refill"` + `refill_count` in the return dict, the help string, and the
    docstring ladder.
...
- **File:** `src/hpc_agent/meta/campaign/atoms/load_context.py`.
  - `_next_step_hint` (its `in_flight == 0` gate) only emits `decide` when `in_flight == 0`. Change
    its signature to receive the manifest flags (`async_refill`, `max_in_flight`) — thread from the
    caller `load_context`. When async + a campaign exists + **per-campaign** `in_flight < K`,
    return `decide`/`refill` **even while `in_flight > 0`**, while still returning
    `monitor`/`aggregate` for in-flight runs that need them. Per-campaign slot count from the
    `in_flight` rows (they carry `campaign_id`).
  - `_build_delegate`'s decide arm: reuse it for refill (the resolver dispatches on
...
- **File:** `src/hpc_agent/meta/campaign/deterministic_resolver.py`.
  - `_resolve_decide`: **before** the `decision != "continue"` terminal gate,
    add `if decision == "refill":` → loop `_submit_next_iteration` `refill_count` times;
    ...
    worst of N, any residue surfaced). Cursor advances once per call → N advances, correct.
...
- **File:** `src/hpc_agent/execution/mapreduce/templates/scaffolds/optuna_strategy.py`.
  - `_propose`: tell by `rec["trial_tokens"]` (out-of-order safe) instead of
    oldest-first index; add a `constant_liar` sampler to `create_study`. **Keep
...
- **Per-cluster seed blocks:** each cid's manifest `strategy.params` carries that cluster's
  ...
  (`incorporation/build/submit_spec.py::_campaign_strategy_kw_env`). Document the pattern.
...
- `_next_step_hint` has **no** manifest/`experiment_dir` access today — the signature change
  ripples to its `load_context` call site.
