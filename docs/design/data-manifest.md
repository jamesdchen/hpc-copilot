...
`infra/prune.py::plan_prune` (→ `PrunePlan`): it splits the manifest
...
kill-switch `HPC_NO_DEPLOY_PRUNE=1`. Wired into the `infra/transport/` package's
...
nothing pruned). Tests: `tests/infra/test_prune.py` (8, the planner)
and `tests/infra/test_transport_prune.py` (10, the seam: manifest-known
