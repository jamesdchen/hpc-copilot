...
**2026-07-27 — the MCP-elicitation sign-off popup RETIRED; the inline chat relay is the read-and-sign surface (user-ruled).** Every popup-era entry above (the E-render digest, popup-primary, item 12's declared-but-dark degradation, the finding-12 "popup-primary" framing) is design HISTORY now: the elicitation channel was removed wholesale (`docs/design/mcp-elicitation.md`, drift-log final entry — client-discretion rendering could not carry the audit). The surviving flow: `notebook-audit-view` still defaults to the DIGEST markdown, and `full: true` is now the INLINE read-and-sign relay — the whole-body render with commented-out exposition runs collapsed by CODE to disclosed elision lines (`audit_view._elide_exposition_diff`; the on-disk `render_path` file keeps the full exposition for out-of-chat auditing). The human reads that inline relay (or the render file) and types the sign-off in chat; the T8 gate's evidence tiers are unchanged (the popup only ever fed the same utterance log the capture hook feeds). The skill's rendezvous guidance re-pinned: `tests/contracts/test_notebook_audit_skill_guidance.py::test_chat_signoff_is_the_signoff_surface_not_the_popup`.
