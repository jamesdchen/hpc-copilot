...
atomic-ops layer (the per-subject runners under `hpc_agent/ops/`)
that the slash commands use, so the invariants in
[`docs/internals/sync-checklist.md`](../internals/sync-checklist.md)
bind both.
...
- `_PACKAGE_ROOT` — absolute path to the **package directory**
  ...
  under `hpc_agent/execution/mapreduce/templates/runtime/<scheduler>/`, and
  starter scaffolds under `hpc_agent/execution/mapreduce/templates/scaffolds/`.
...
- `hpc_agent.execution.mapreduce.reduce.history.prior(experiment_dir, campaign_id)` —
...
- `hpc_agent.execution.mapreduce.reduce.history.find_sidecars_by_campaign` /
...
- **`.hpc/tasks.py`** — the user-written Python module exposing
  ...
  `hpc_agent/execution/mapreduce/templates/scaffolds/tasks_example.py`), git-tracked, and
...
2. **`hpc_agent/**` MUST NOT import from
   `hpc_agent/execution/mapreduce/templates/`.** Templates are source files
   copied into experiment repos; treating them as importable modules
   would couple the framework to a fixed set of templates.
3. **`hpc_agent/execution/mapreduce/templates/**` MUST NOT import from
   ...
   - `hpc_agent.execution.mapreduce.metrics_io` — the `write_metrics` sidecar
   ...
   - `hpc_agent.executor_cli` — the `flag`, `generic_args`,
     ...
     `hpc_agent/execution/mapreduce/templates/scaffolds/tasks_example.py`) and
...
- **Per-task isolation.** Each task runs in a fresh subprocess
  (`execution/mapreduce/dispatch.py:Popen`); no in-process state leaks across
...
- **Deterministic env defaults.** The cluster preamble
  (`hpc_agent/execution/mapreduce/templates/runtime/common/hpc_preamble.sh`) exports
...
- **Order-invariant aggregation.** The combiner
  (`execution/mapreduce/combiner.py`) uses `sorted()` for grid-key and
