...
- `stages.input.json` — output shape of `.hpc/stages.py::stages()` (loaded by `claude_hpc.orchestrator.planning.stages.load_stages`; not a CLI input but agents authoring `stages.py` should validate against this).
