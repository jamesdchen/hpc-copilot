"""Tests for aggregate_metrics in chunk_loader."""
...
class TestAggregateMetrics:
    def test_weighted_mean(self, tmp_path: Path) -> None:
        ...
        result = aggregate_metrics(tmp_path, total_chunks=3)
    ...
    def test_no_n_samples_equal_weight(self, tmp_path: Path) -> None:
        ...
        result = aggregate_metrics(tmp_path, total_chunks=2)
        assert result["loss"] == pytest.approx(3.0)
        assert "n_samples" not in result

    def test_empty_directory(self, tmp_path: Path) -> None:
        assert aggregate_metrics(tmp_path, total_chunks=3) == {}
