...
* ``detect_experiment_tier`` infers Tier-1 / Tier-2 from path layout + markers.
...
def _write_meta_json(root: Path) -> None:
    (root / "meta.json").write_text(
        '{"experiment_id": "x", "seed": 42, "purpose": "t"}\n', encoding="utf-8"
    )


# ─── Item 1: meta.json layout filter ──────────────────────────────────────


class TestMetaJsonLayoutFilter:
    def test_meta_present_skips_src(self, tmp_path: Path) -> None:
        _write_meta_json(tmp_path)
...
class TestDetectExperimentTier:
