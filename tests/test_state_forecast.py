"""Tests for ``hpc_mapreduce.job.state_forecast.forecast_state_at``."""

from __future__ import annotations

from claude_hpc._internal._time import utcnow_iso
from claude_hpc.infra.inspect import ClusterSnapshot, NodeSnapshot
from hpc_mapreduce.job.state_forecast import forecast_state_at
            ...
            _node(co_tenants=[
                {
                    ...
                    "cpus": 8, "mem_gb": 32, "gpus": 1,
                    "elapsed_s": 100, "walltime_ask_sec": 3600,
                }
            ])
        ])
...
class TestCompletionsByT:
    def test_long_running_jobs_complete_by_large_t(self):
        prof = UserProfile(
            user="alice", n_observations=50, median_actual_over_ask=0.5,
        )
        snap = _snap([
            _node(co_tenants=[
                {
                    "user": "alice", "state": "RUNNING",
                    "cpus": 8, "mem_gb": 32, "gpus": 1,
                    "elapsed_s": 100, "walltime_ask_sec": 1000,
                }
            ])
        ])
        out = forecast_state_at(
            snap, t_offset_sec=1000, profiles={"alice": prof},
        )
    ...
    def test_jobs_outliving_t_not_counted(self):
        prof = UserProfile(
            user="alice", n_observations=50, median_actual_over_ask=1.0,
        )
        snap = _snap([
            _node(co_tenants=[
                {
                    "user": "alice", "state": "RUNNING",
                    "cpus": 8, "mem_gb": 32, "gpus": 1,
                    "elapsed_s": 0, "walltime_ask_sec": 10000,
                }
            ])
        ])
        out = forecast_state_at(
            snap, t_offset_sec=100, profiles={"alice": prof},
        )
        assert out.n_jobs_completing_by_t == 0
        assert out.available_gpus == 3
    ...
    def test_mid_run_partial_availability(self):
        prof_short = UserProfile(
            user="alice", n_observations=50, median_actual_over_ask=0.4,
        )
        prof_long = UserProfile(
            user="bob", n_observations=50, median_actual_over_ask=1.0,
        )
        snap = _snap([
            _node(co_tenants=[
                {
                    "user": "alice", "state": "RUNNING",
                    "cpus": 4, "mem_gb": 16, "gpus": 1,
                    "elapsed_s": 200, "walltime_ask_sec": 1000,
                },
                {
...
class TestColdStart:
    def test_unprofiled_user_uses_fallback(self):
        snap = _snap([
            _node(co_tenants=[
                {
                    "user": "stranger", "state": "RUNNING",
                    "cpus": 4, "mem_gb": 16, "gpus": 1,
                    "elapsed_s": 0, "walltime_ask_sec": 1000,
                }
            ])
        ])
...
class TestEdge:
    def test_drained_node_excluded_from_capacity(self):
        snap = _snap([
            NodeSnapshot(
                name="x", gres="gpu:8", cpu_tot=64, real_mem_mb=256000,
                is_drained=True,
            ),
            _node(),
        ])
