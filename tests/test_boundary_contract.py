...
import claude_hpc
from claude_hpc.orchestrator.state.discover import _SKIP_BASENAMES, _SKIP_DIRS
...
def _imported_dotted_modules(path: Path) -> set[str]:
    ...
    runtime module) from ``claude_hpc.orchestrator.state.runs`` (framework-internal).
    """
    source = path.read_text(encoding="utf-8")
    try:
        tree = ast.parse(source, filename=str(path))
