...
spec with EXACTLY two deterministic flips and nothing else changed —
...
(#283: the former ``skip_rsync_deploy: true`` flip was dropped — that
bypass is now operator/internal-only and off the wire ``SubmitFlowSpec``,
so a wire spec this verb emits cannot carry it. The in-process two-phase
path skips the redundant deploy via ``HPC_AGENT_SKIP_RSYNC_DEPLOY`` / a
Python-only kwarg instead.)
...
def _phase1_spec(**overrides: Any) -> dict[str, Any]:
    ...
    (``canary=true``, ``canary_only=true``). Overrides are merged on top so
    a test can mutate a single field.
    """
    spec: dict[str, Any] = {
        "profile": "train",
        "cluster": "hoffman2",
...
class TestFlips:
    def test_two_flips_applied(self) -> None:
        ...
        # #283: skip_rsync_deploy is no longer a flip — it is off the wire spec
        # entirely, so the derived Phase-2 spec must not carry it.
        assert "skip_rsync_deploy" not in phase2

    def test_every_other_field_preserved(self) -> None:
        spec = _phase1_spec()
        out = prepare_phase2_spec(spec=spec)
        phase2 = out["phase2_spec"]
        flipped = {"canary", "canary_only"}
...
class TestFlipsApplied:
    def test_reports_the_two_flips(self) -> None:
        out = prepare_phase2_spec(spec=_phase1_spec())
...
class TestPhase2Validates:
    ...
    def test_phase1_skip_rsync_deploy_field_is_refused(self) -> None:
        # #283: a hand-authored skip_rsync_deploy on the Phase-1 spec is now a
        # wire-refused field. prepare-phase2-spec validates the derived spec
        # against SubmitFlowSpec, so it surfaces a typed SpecInvalid rather than
        # silently forwarding the bypass.
        with pytest.raises(errors.SpecInvalid):
            prepare_phase2_spec(spec=_phase1_spec(skip_rsync_deploy=True))
