...
def _wire(monkeypatch, *, record, scheduler: str, ssh_stdout: str = "", ssh_rc: int = 0) -> None:
    ...
    monkeypatch.setattr(
        "hpc_agent.infra.remote.ssh_run", lambda cmd, **k: _cp(stdout=ssh_stdout, rc=ssh_rc)
    )
