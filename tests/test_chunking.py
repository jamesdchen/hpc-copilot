"""Tests for hpc_mapreduce.map.context protocol."""
...
from hpc_mapreduce.map.context import MapContext, map_context, collect_outputs
...
class TestMapContext:
    ...
    def test_split_single_chunk_returns_full_range(self):
        ctx = MapContext(chunk_id=0, total_chunks=1, result_dir=Path("."))
    ...
    def test_split_even_division(self):
        ...
        ranges = []
        for i in range(4):
            ctx = MapContext(chunk_id=i, total_chunks=4, result_dir=Path("."))
    ...
    def test_split_with_remainder(self):
        ...
        ranges = []
        for i in range(3):
            ctx = MapContext(chunk_id=i, total_chunks=3, result_dir=Path("."))
    ...
    def test_split_preserves_offset(self):
        ...
        ctx = MapContext(chunk_id=0, total_chunks=2, result_dir=Path("."))
        r = ctx.split(range(50, 150))
        assert r.start == 50
        assert r.stop == 100
        ctx2 = MapContext(chunk_id=1, total_chunks=2, result_dir=Path("."))
    ...
    def test_split_covers_all_indices_with_offset(self):
        ...
        all_indices = []
        for i in range(7):
            ctx = MapContext(chunk_id=i, total_chunks=7, result_dir=Path("."))
            all_indices.extend(ctx.split(items))
        assert sorted(all_indices) == list(items)

    def test_output_path_default(self):
        ctx = MapContext(chunk_id=4, total_chunks=10, result_dir=Path("/results/run1"))
...
class TestCollectOutputs:
    ...
    def test_custom_pattern(self, tmp_path):
        ...
        df = collect_outputs(tmp_path, pattern="output_*.csv")
        assert len(df) == 3


class TestCounters:
    def test_counter_path(self):
        ctx = MapContext(chunk_id=4, total_chunks=10, result_dir=Path("/results"))
        assert ctx.counter_path() == Path("/results/_counters_5.json")

    def test_update_counters_writes_json(self, tmp_path):
        ctx = MapContext(chunk_id=0, total_chunks=1, result_dir=tmp_path)
        ctx.emit_counters(rows_processed=5000, errors=2)
        import json
    ...
    def test_read_counters(self, tmp_path):
        ctx = MapContext(chunk_id=0, total_chunks=1, result_dir=tmp_path)
        ctx.emit_counters(rows_processed=42, total_rows=100)
    ...
    def test_update_counters_overwrites(self, tmp_path):
        ctx = MapContext(chunk_id=0, total_chunks=1, result_dir=tmp_path)
        ctx.emit_counters(rows_processed=10, errors=1)
        ctx.emit_counters(rows_processed=50, errors=3)
        result = ctx.read_counters()
        assert result == {"rows_processed": 50, "errors": 3}

    def test_update_counters_atomic(self, tmp_path):
        ctx = MapContext(chunk_id=0, total_chunks=1, result_dir=tmp_path)
        ctx.emit_counters(rows_processed=100)
