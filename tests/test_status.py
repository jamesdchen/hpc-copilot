"""Tests for hpc.status — aggregate_counters and report_status."""
...
from hpc.status import aggregate_counters, report_status


class TestAggregateCounters:
    def test_aggregate_counters_basic(self, tmp_path):
        ...
        result = aggregate_counters(tmp_path, 3)
    ...
    def test_aggregate_counters_missing_files(self, tmp_path):
    ...
    def test_aggregate_counters_corrupt_file(self, tmp_path):
    ...
    def test_aggregate_counters_empty_dir(self, tmp_path):
        result = aggregate_counters(tmp_path, 3)
        assert result["reporting"] == 0
        assert result["totals"] == {}
        assert result["progress"] is None

    def test_aggregate_counters_no_progress_keys(self, tmp_path):
        for i in range(1, 4):
            (tmp_path / f"_counters_{i}.json").write_text(
    ...
    def test_report_status_includes_counters(self, tmp_path):
        # Write counter files so aggregate_counters finds them.
        (tmp_path / "_counters_1.json").write_text(
            json.dumps({"rows_processed": 10, "total_rows": 50})
        )

        # Patch scheduler query functions to avoid real subprocess calls.
        with patch("hpc.status.detect_scheduler", return_value="slurm"), \
             patch("hpc.backends.query.query_sacct", return_value={}):
