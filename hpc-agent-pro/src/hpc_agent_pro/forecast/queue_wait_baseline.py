...
a :class:`~hpc_agent_pro.forecast.queue_features.QueueFeatures` snapshot
...
def predict_queue_wait(
    ...
    * ``"des"`` — discrete-event simulator
      (:mod:`hpc_agent_pro.forecast.queue_simulator`) running forward from
    ...
    * ``"auto"`` (default) — DES when a recent snapshot exists AND user
    ...
    candidate:
        Optional :class:`~hpc_agent_pro.forecast.queue_simulator.SimJob`.
    ...
    seed = spec.seed
    from hpc_agent_pro.forecast.queue_wait_des import _des_eligible, _predict_des
...
def __getattr__(name: str):  # noqa: ANN202
    if name in (
        ...
        from hpc_agent_pro.forecast import queue_wait_des
