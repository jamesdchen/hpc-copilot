...
* **`diurnal_ma`** — the v1 baseline. Reads the runtime-prior pool for
  ...
  `hpc_agent_pro.forecast.queue_wait_baseline._predict_diurnal_ma`.
* **`des`** — Phase 4 discrete-event simulator. Loads the most recent
  ...
  `hpc_agent_pro.forecast.queue_simulator` +
  `hpc_agent_pro.forecast.queue_simulator_inputs`.
...
`hpc_agent_pro.forecast.queue_wait_baseline._des_eligible`.
