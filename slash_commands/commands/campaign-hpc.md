...
A campaign-aware `tasks.py` reads prior iterations' reduced metrics to decide what to run next. Three patterns are common; see `docs/campaign.md` for full snippets you can paste verbatim. Show the user:

- **Random search** (no extra dependencies): `random.uniform(...)` over a parameter space, stops after N iterations.
- **Optuna ask/tell**: `import optuna; study = optuna.load_study(...); study.ask().params`. The user must already have Optuna installed; the framework does not bundle it.
- **Walk-forward** (deterministic schedule): index into a list of time windows by `len(_PRIOR)`; stop when the list is exhausted.

The chosen pattern fills the body of `.hpc/tasks.py`'s `total()` and `resolve(i)` — same convention as today, just with `_PRIOR = prior(".", os.environ["HPC_CAMPAIGN_ID"])` at the top of the module.

If the user has an existing `.hpc/tasks.py` and is converting it to a campaign, walk them through adding the `_PRIOR` bootstrap and the `len(_PRIOR) >= N -> total()=0` stopping rule. Don't rewrite their `resolve(i)` body without permission.
